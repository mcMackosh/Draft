...
public class TagController : ControllerBase
{
    ...
    public async Task<IActionResult> GetTagsByProjectId()
    {
        var projectId = GetProjectId();
        if (projectId == 0)
        {
    ...
    [HttpPut]
    [ProjectAuthorization(RoleEnum.Admin, RoleEnum.Manager)]
    public async Task<IActionResult> UpdateTag([FromQuery] int tagId, [FromBody] UpdateTagDto tagDto)
    {
        var projectId = GetProjectId();
        if (projectId == 0 || !await _tagService.IsTagHaveCorrectProjectId(projectId, tagId))
        {
            return BadRequest(new ApiResponse<string>(400, "Project not chosen or invalid tag for this project"));
        }
    ...
    public async Task<IActionResult> DeleteTag([FromQuery] int tagId)
    {
        ...
        try
        {
            ...
            return Ok(new ApiResponse<string>(200, "Tag deleted"));
        }
        catch (Exception ex)
        {
            return BadRequest(new ApiResponse<string>(400, ex.Message));
